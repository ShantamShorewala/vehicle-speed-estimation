...
def sample_with_heatmap(inp, out, num_rows=2, parts_to_show=None, coords=None):
    ...
    fuller = cv2.cvtColor(full_img, cv2.COLOR_RGB2BGR)
    cv2.imwrite("/home/shantam/Documents/pred.png", fuller)
    return full_img

def batch_with_heatmap(inputs, outputs, mean=torch.Tensor([0.5, 0.5, 0.5]).cuda(), num_rows=2, parts_to_show=None, coords = None):
    batch_img = []
    #print ('out size', outputs.size(0), outputs.size(1), outputs.size(2))
