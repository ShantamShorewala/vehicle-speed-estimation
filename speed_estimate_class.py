...
class speed_estimation:
	...
	def reproject_error(pose, points, imagepoints): 
	#arguments: pose (rvecs, tvecs), world_points, image_points
		global weights
	...
	def array_dump(self, filename, img_coords, real_coords):
		with open(filename, "w") as arraydump:
			json.dump({"image":list(img_coords),"world":list(real_coords)}, codecs.open(filename, 'w', encoding='utf-8'), sort_keys=True, indent=4)
