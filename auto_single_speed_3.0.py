...
out = cv2.VideoWriter('check.avi',fourcc, 20.0, (1920,1080))
...
def compute_pose(points, imagepoints):
	...
	print("total error: {}".format(mean_error/len(points)))
	old_error = mean_error/len(points)

	imagepoints_finetuned = finetune(old_error, points, imagepoints, pts, mtx, dist, criteria, iterations=100)

	for i in range(len(imagepoints_finetuned)):
		twist[i] = [float(imagepoints_finetuned[i][0]), float(imagepoints_finetuned[i][1])]
	...
	print( "reduced error: {}".format(old_error - mean_error/len(points)), mean_error/len(points))
	#time.sleep(5)
	return fin_pose
